...
class VectorStore:
    ...
    async def vector_search(self, query_embedding: list[float], top_k: int = 4) -> list[dict]:
        ...
        # Ensure embedding dimensions match index expectations (768)
        if len(query_embedding) != self.EMBEDDING_DIMENSIONS:
            if len(query_embedding) < self.EMBEDDING_DIMENSIONS:
                query_embedding = query_embedding + [0.0] * (self.EMBEDDING_DIMENSIONS - len(query_embedding))
            else:
                query_embedding = query_embedding[:self.EMBEDDING_DIMENSIONS]
    ...
    async def keyword_search(self, query_text: str, top_k: int = 4) -> list[dict]:
        ...
        # Fallback: MongoDB Regex search if $search index is absent or returns 0
        try:
            keywords = [w for w in query_text.replace("?", "").replace(",", "").split() if len(w) > 1]
            regex_pattern = "|".join(keywords) if keywords else query_text
            cursor = collection.find({"text": {"$regex": regex_pattern, "$options": "i"}}).limit(top_k)
            ...
            logger.warning(f"MongoDB Regex fallback search failed: {ex}")
            return []
